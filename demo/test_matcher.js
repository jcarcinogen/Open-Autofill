...
const noRadio = { ...radioBase, id: "no", label: "No", value: "false" };
assert(M.radioPersistValue(noRadio, "false") === "No", "a selected radio whose HTML value is false persists its label");
const learnedNoRadio = M.learnFromField(noRadio, "false", {}, [], settings);
assert(learnedNoRadio.siteFields[0].value === "No", "selected false-valued radio is remembered");
assert(M.resolveValue(noRadio, {}, learnedNoRadio.siteFields, settings).source === "site", "selected false-valued radio replays");
const labelLessNoRadio = { ...radioBase, id: "no-label", label: "", value: "false" };
assert(M.shouldRememberCurrentValue(labelLessNoRadio, "false", {}, settings), "selected false-valued radio without a label remains rememberable");
const learnedLabelLessNo = M.learnFromField(labelLessNoRadio, "false", {}, [], settings);
assert(learnedLabelLessNo.siteFields[0].value === "false", "label-less false-valued radio persists its literal option value");
const sameLabelCheckbox = { ...radioBase, type: "checkbox", name: "consent", id: "consent", label: "No", value: "on", checked: false };
assert(!M.resolveValue(sameLabelCheckbox, {}, learnedNoRadio.siteFields, settings).source, "radio records cannot leak into same-label checkboxes");
